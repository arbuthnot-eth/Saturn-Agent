...
def set_all_loggers_to_level(level: str):
    """Set all loggers to the same level, affecting future logger creations or by reconfiguring existing ones (if supported)."""
    config = get_logging_config()
    config.set_all_loggers_level(level)

def get_all_logger_env_vars() -> Dict[str, str]:
    """Get all defined environment variables for logger levels and their current values."""
    config = get_logging_config()
    return config.get_all_env_vars()

# New global function to be called by WebServer.py
def close_logs():
    """Initiates the shutdown of the asynchronous log processor if it's running."""
    global _async_log_processor
    if _async_log_processor:
        logger = get_configured_logger(__name__) # Get a logger for this utility
        logger.info("Global close_logs() called, shutting down async log processor.")
        _async_log_processor.shutdown()
    else:
        # If the processor was never started, there's nothing to do, but log it for clarity
        # This might happen if no LazyLoggers were ever actually used to log anything.
        # Or if the application terminates very early.
        # We get a temporary direct logger here to avoid circular dependencies or issues if logging is not fully up.
        temp_logger = get_logging_config().get_logger(__name__) # Get a direct logger instance for this specific message
        temp_logger.info("Global close_logs() called, but async log processor was not initialized or already shut down.")
