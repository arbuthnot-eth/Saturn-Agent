...
class NLWebHandler:
    ...
    async def prepare(self):
        ...
        if not self.retrieval_done_event.is_set():
            ...
            logger.critical(f"CRITICAL_BASEHANDLER_LOG: About to call client.search with:")
            logger.critical(f"CRITICAL_BASEHANDLER_LOG:   query (self.decontextualized_query): {self.decontextualized_query}")
            logger.critical(f"CRITICAL_BASEHANDLER_LOG:   site (self.site): {self.site}")
            items = await client.search(query=self.decontextualized_query, site=self.site, num_results=50)
