...
class AppConfig:
    ...
    def load_embedding_config(self, path: str = "config_embedding.yaml"):
        ...
        for name, cfg in data.get("providers", {}).items():
            ...
            # Prioritize direct keys, then _env suffixed keys
            api_key = self._get_config_value(cfg.get("api_key"), cfg.get("api_key_env"))
            api_endpoint = self._get_config_value(cfg.get("endpoint"), cfg.get("api_endpoint_env"))
            api_version = self._get_config_value(cfg.get("api_version"), cfg.get("api_version_env"))
            model = self._get_config_value(cfg.get("model"))
            gateway_id = self._get_config_value(cfg.get("gateway_id"), cfg.get("gateway_id_env"))
            base_url = self._get_config_value(cfg.get("base_url"))
    ...
    def load_retrieval_config(self, path: str = "config_retrieval.yaml"):
        ...
        for name, cfg in data.get("endpoints", {}).items():
            ...
            self.retrieval_endpoints[name] = RetrievalProviderConfig(
                api_key=self._get_config_value(cfg.get("api_key"), cfg.get("api_key_env")),
                api_endpoint=self._get_config_value(cfg.get("api_endpoint"), cfg.get("api_endpoint_env")),
                database_path=self._get_config_value(cfg.get("database_path")),
                index_name=self._get_config_value(cfg.get("index_name")),
                db_type=self._get_config_value(cfg.get("db_type")),
                account_id=self._get_config_value(cfg.get("account_id"))
            )
    ...
    def load_nlweb_config(self, path: str = "config_nlweb.yaml"):
        ...
        self.nlweb = NLWebConfig(
            ...
            default_site=self._get_config_value(data.get("default_site"), "all")
        )
