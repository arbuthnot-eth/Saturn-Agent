...
# Corrected: Add logger import and initialization
    ...
    try:
        ...
        for episode in podcast_episodes:
            ...
            if not url and "name" in episode:
                ...
                logger.info(f"Generated synthetic URL for episode: {episode['name']}")
            elif not url:
                # Skip items without any identifiable information
                continue
            
            # Convert to JSON - ensure no newlines in the JSON
            # schema_json = json.dumps(episode, ensure_ascii=False).replace("\n", " ") # We'll use specific fields for metadata

            name = episode.get("name", "Untitled Episode")
            # Attempt to get page_content from description or content fields
            page_content = episode.get("description", "") # Default to description
            if not page_content and "content" in episode and isinstance(episode["content"], list) and episode["content"]:
                # Handle cases where content might be a list of dictionaries (e.g., content:encoded)
                # We'll take the first one's value if available.
                content_item = episode["content"][0]
                if isinstance(content_item, dict) and "value" in content_item:
                    page_content = content_item.get("value", "")
            elif not page_content and "summary" in episode: # Another common field
                page_content = episode.get("summary", "")

            publish_date = episode.get("datePublished", episode.get("published", "")) # Try common date fields

            # Create document with the new structure
            document = {
                "id": str(hash(url) % (2**63)),  # Create a stable ID from the URL
                # "schema_json": schema_json, # No longer storing the full schema_json in this top-level dict for this purpose
                                            # It will be part of the metadata if needed or handled by chunker.
                                            # For now, we are focusing on what vectorize_client needs.
                "page_content": page_content, # This is what vectorize_client expects for 'text'
                "metadata": {
                    "url": url,
                    "title": name,
                    "publish_date": publish_date,
                    # site is added by vectorize_client from its site_name param
                    # original_doc_id is added by vectorize_client from the top-level 'id'
                },
                "site": site # Keep site here for consistency if other parts of db_load use it
            }
            
            documents.append(document)
        
        logger.info(f"Processed {len(documents)} episodes from RSS/Atom feed")
        return documents
    except Exception as e:
        logger.error(f"Error processing RSS/Atom feed: {str(e)}")
        traceback.print_exc()
        return []
            ...
            try:
                ...
                if len(batch_documents) >= batch_size or i == total_lines - 1:
                    if batch_documents:
                        ...
                        await client.upload_documents(batch_documents, site_name=site)
...
async def loadJsonToDB(file_path: str, site: str, batch_size: int = 100, delete_existing: bool = False, force_recompute: bool = False, database: str = None):
    ...
    try:
        ...
        # ---- START DEBUG CODE FOR EXACT STRING EMBEDDING ----
        if provider == "cloudflare": 
            try:
                logger.critical("CRITICAL_DBLOAD_EMBED_TEST: Attempting to embed 'latest updates on trump' during data load.")
                from embedding.cloudflare_embedding import get_cloudflare_embedding 

                test_query_text_debug = "latest updates on trump"
                
                if model:
                    test_query_embedding_debug = await get_cloudflare_embedding(test_query_text_debug, model=model)
                else:
                    logger.error("CRITICAL_DBLOAD_EMBED_TEST_ERROR: model is None, cannot test.")
                    test_query_embedding_debug = None

                if test_query_embedding_debug:
                    logger.critical(f"CRITICAL_DBLOAD_EMBED_TEST_RESULT: Query '{test_query_text_debug}', Embedding (first 5): {test_query_embedding_debug[:5]}")
                else:
                    logger.error("CRITICAL_DBLOAD_EMBED_TEST_RESULT: Failed to get embedding for test query (it was None or empty).")
            except ImportError as ie_debug:
                logger.error(f"CRITICAL_DBLOAD_EMBED_TEST_ERROR: ImportError during test embedding: {ie_debug}. Check import path for get_cloudflare_embedding.")
            except Exception as e_test_embed_debug:
                logger.error(f"CRITICAL_DBLOAD_EMBED_TEST_ERROR: Exception during test embedding: {e_test_embed_debug}")
                logger.error(traceback.format_exc())
        # ---- END DEBUG CODE FOR EXACT STRING EMBEDDING ----
        ...
        if all_documents:
            ...
            with open(embeddings_path, 'w', encoding='utf-8') as embed_file:
                ...
                texts = [doc.get("page_content", "") for doc in all_documents]
                        ...
                        try:
                            ...
                            for j, embedding in enumerate(embeddings):
                                if j < len(batch_docs):
                                    ...
                                    # doc_json = doc['schema_json'].replace('\n', ' ') # OLD LINE - doc might not have schema_json
                                    
                                    # Write to embeddings file
                                    # embed_file.write(f"{doc['url']}\t{doc_json}\t{embedding_str}\n") # OLD LINE - doc might not have top-level url or schema_json

                                    # MODIFIED: Use URL from metadata and page_content for the text part
                                    item_url_for_cache = doc.get("metadata", {}).get("url", "")
                                    item_text_for_cache = doc.get("page_content", "").replace('\n', ' ')
                                    embed_file.write(f"{item_url_for_cache}\t{item_text_for_cache}\t{embedding_str}\n")
                            ...
                            await client.upload_documents(docs_with_embeddings, site_name=site)
                            print(f"Successfully uploaded batch {batch_idx+1}")
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            if has_embeddings and not args.force_recompute:
                ...
                # 1. The file does not have pre-computed embeddings (has_embeddings is False).
                # 2. The file has pre-computed embeddings, but force_recompute is True.
                # In both scenarios, embeddings need to be (re)computed.

                # Determine embedding provider and model
                embedding_provider_name = CONFIG.preferred_embedding_provider
                embedding_config = CONFIG.get_embedding_provider(embedding_provider_name)
                embedding_model_name = embedding_config.model if embedding_config else None
                logger.info(f"Using embedding provider: {embedding_provider_name}, model: {embedding_model_name}")

                # ---- START DEBUG CODE FOR EXACT STRING EMBEDDING ----
                if embedding_provider_name == "cloudflare": 
                    try:
                        logger.critical("CRITICAL_DBLOAD_EMBED_TEST: Attempting to embed 'latest updates on trump' during data load.")
                        from embedding.cloudflare_embedding import get_cloudflare_embedding 
                        ...
                        if embedding_model_name:
                            test_query_embedding_debug = await get_cloudflare_embedding(test_query_text_debug, model=embedding_model_name)
                        else:
                            logger.error("CRITICAL_DBLOAD_EMBED_TEST_ERROR: embedding_model_name is None, cannot test.")
                            test_query_embedding_debug = None
                ...
                logger.info("File does not have pre-computed embeddings or recomputation is forced. Will compute/recompute and load.")
